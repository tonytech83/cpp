...
int main() {
    ...
    for (int i = 0; i < PLAYERS_COUNT; ++i) {
        ...
        players.emplace_back(playerHealth, playerArmor, i);
    }
    ...
    for (int i = 0; i < PLAYERS_COUNT; ++i) {
        std::cin >> pistolId >> pistolDamagePerRound >> pistolClipSize >> pistolRemainingAmmo;

        players[i].buyPistol(static_cast<PistolType>(pistolId), pistolDamagePerRound, pistolClipSize,
                             pistolRemainingAmmo);
    }

...
public:
    Lecturer(std::string name, double rating) : name(name), rating(rating) { NumLecturers++; }

    ~Lecturer() { NumLecturers--; }

    static int getNumLecturers() { return NumLecturers; }
...
void print(Lecturer lecturer) { std::cout << lecturer.getName() << " " << lecturer.getRating() << std::endl; }

void showAndClearLecturers() {
    Lecturer a("Dandelion", 6.9), b("Geralt", 1.3), c("Yen", 4.2);

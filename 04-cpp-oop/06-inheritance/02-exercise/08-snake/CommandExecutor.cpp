...
static const char *DIR_STRINGS[DIRECTIONS_COUNT]{"UP", "RIGHT", "DOWN", "LEFT"};
...
void CommandExecutor::extractCommand(const std::string &commandStr) {
    ...
    switch (currCommand) {
        ...
        default:
            std::cerr << "Warning, received unknown InputCommand: " << currCommand << std::endl;
            break;
    }
}

bool CommandExecutor::isGameActive() const { return _isGameActive; }

void CommandExecutor::executeMoveSnake(const int moveDirectionId) {
    std::cout << "MOVE_SNAKE in dir: " << DIR_STRINGS[moveDirectionId] << ", status: ";

    const StatusCode STATUS_CODE = _field.moveSnake(static_cast<Direction>(moveDirectionId));
...
void CommandExecutor::executeGenerateObstacle(const Point &pos) {
    std::cout << "GENERATE_OBSTACLE at row: " << pos.row << ", col: " << pos.col << '\n';
...
void CommandExecutor::executeGeneratePowerUp(const Point &pos) {
    std::cout << "GENERATE_POWER_UP at row: " << pos.row << ", col: " << pos.col << '\n';
